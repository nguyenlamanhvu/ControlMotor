...
/* USER CODE BEGIN PV */
// Variables for PID
float K_p = 0.28; 	//k_p = 0.28
float K_i = 0.0001; //k_i = 0.0001
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	if(huart->Instance == huart2.Instance){
		...
		if(main_buff[0] == 0x0A && main_buff[1] == (uint8_t)(Size-3) && main_buff[Size-1] == 0x05){
